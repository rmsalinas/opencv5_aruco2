...
// #include "../precomp.hpp" use only into opencv
...
public:
  ...
  static inline std::vector<FiducialMarker>
  detect(const cv::Mat &img, const std::vector<DictionaryType> dictionaries,
         const DetectionParameters &params = DetectionParameters(),
         std::vector<FiducialMarker> *candidatesOut = nullptr,
         cv::Mat ThresImageIn = {});
  static inline std::vector<FiducialMarker>
  detect(const cv::UMat &img, const std::vector<DictionaryType> dictionaries,
         const DetectionParameters &params = DetectionParameters(),
         std::vector<FiducialMarker> *candidatesOut = nullptr,
         cv::UMat ThresImageIn = {});

private:
  static inline FiducialMarker sort(const FiducialMarker &marker);
  static inline float getSubpixelValue(const cv::Mat &im_grey,
                                       const cv::Point2f &p);
  static inline int getMarkerId(cv::Mat candidateBits, int &idx,
                                int &nrotations,
                                const DetectionParameters &params,
                                Dictionary &dictionary);
  static inline int isInto(const std::vector<cv::Point2f> &a,
                           const std::vector<cv::Point2f> &b);
  static std::vector<std::vector<cv::Point>>
  visitedAwareTracingContour(cv::Mat &padded_io, size_t minSize = 1,
                             float maxRevisited = 0.1);
...
// Marker intersection. Tells the marker with most corners into another. 0 if no
// intersection or tie
int MarkerDetector::isInto(const std::vector<cv::Point2f> &a,
                           const std::vector<cv::Point2f> &b) {
...
std::vector<FiducialMarker> MarkerDetector::detect(
    const cv::UMat &img, const std::vector<DictionaryType> dictionaries,
    const DetectionParameters &params,
    std::vector<FiducialMarker> *candidatesOut, cv::UMat ThresImIn) {
  ...
  // GPU buffers allocated on stack (efficiently recycled by OpenCV's internal pool)
  ...
  if (ThresImIn.empty()) {
      ...
      // Concurrently thresholds the image and initializes the label array for Union-Find
      ...
      // 1. Init labels
      // Initialize the Union-Find label array when thresholded image is pre-provided
      cv::ocl::Kernel k_init_labels("init_labels", program);
      k_init_labels.args(cv::ocl::KernelArg::PtrReadWrite(u_labels), width, height);
      k_init_labels.run(2, globalThreads, NULL, false);
  }


  // 2. Merge (Union-Find)
  // Merges adjacent pixels with the same binary threshold state in the label array
  ...
  // Resolves the equivalence trees so that each pixel points directly to its root component ID
  ...
  // Finds corners within each labeled component and stores them in a spatial hash table
  ...
  k_compact.args(cv::ocl::KernelArg::PtrReadOnly(u_hash_counts),
                 ...
                 max_points_per_hash,
                 MAX_EXPECTED_MARKERS, params.minSize * params.minSize,
  ...
  if (num_polygons > 0) {
    ...
    // 1. Calculate average edge length of candidates to determine refinement window size
    double avrgLen = 0;
    for (int i = 0; i < num_polygons; i++) {
      for (int j = 0; j < 4; j++) {
    ...
    for (size_t di = 0; di < dictionaries.size(); di++) {
      ...
      dict_cache.erase(
          std::remove_if(dict_cache.begin(), dict_cache.end(),
      ...
      if (cached_found) {
        ...
        if (!gpu_dict_codes.empty()) {
          cv::Mat mat_dict_codes(1, (int)gpu_dict_codes.size() * 8, CV_8UC1, (void*)gpu_dict_codes.data());
      ...
      if (!u_dict_codes.empty()) {
        ...
        k_identify.args(
            ...
            (uchar)(params.detectInvertedMarker ? 1 : 0), max_attempts,
            (unsigned int)seed,
        ...
        k_refine.args(
            cv::ocl::KernelArg::PtrReadOnly(u_bwimage), (int)u_bwimage.step,
            width, height,
            cv::ocl::KernelArg::PtrReadOnly(u_valid_corners_out),
            cv::ocl::KernelArg::PtrReadOnly(u_identified_ids),
            cv::ocl::KernelArg::PtrReadOnly(u_identified_rotations),
            cv::ocl::KernelArg::PtrReadWrite(u_candidate_matched),
            num_polygons, max_attempts, (int)di,
            dictInstance.markerSize,
            halfwsize, 12, 0.005f,
            cv::ocl::KernelArg::PtrReadWrite(u_detected_count),
            cv::ocl::KernelArg::PtrReadWrite(u_detected_markers),
            num_polygons);
    ...
    if (detected_count > 0) {
      ...
      for (int i = 0; i < detected_count; i++) {
        ...
        marker.corners[0] = cv::Point2f(ptr_detected[out_base + 2], ptr_detected[out_base + 3]);
        marker.corners[1] = cv::Point2f(ptr_detected[out_base + 4], ptr_detected[out_base + 5]);
        marker.corners[2] = cv::Point2f(ptr_detected[out_base + 6], ptr_detected[out_base + 7]);
        marker.corners[3] = cv::Point2f(ptr_detected[out_base + 8], ptr_detected[out_base + 9]);
            ...
            });
  {
    ...
    for (int i = 0; i < int(DetectedMarkers.size()) - 1; i++) {
      for (int j = i + 1; j < int(DetectedMarkers.size()) && !toRemove[i];
           j++) {
        // 1. Check if they are nearly identical in position (same or different IDs/dictionaries)
        float dist = 0;
        for (int c = 0; c < 4; c++) {
          dist += cv::norm(DetectedMarkers[i].corners[c] - DetectedMarkers[j].corners[c]);
        }
        if (dist < 5.0f) {
          toRemove[j] = true;
          continue;
...
std::vector<FiducialMarker> MarkerDetector::detect(
    const cv::Mat &img, const std::vector<DictionaryType> dictionaries,
    const DetectionParameters &params,
    std::vector<FiducialMarker> *candidatesOut, cv::Mat ThresImIn) {
  ...
  if (ThresImIn.empty()) {
    cv::boxFilter(bwimage, thresImage, bwimage.type(),
                  cv::Size(params.boxFilterSize, params.boxFilterSize),
                  cv::Point(-1, -1), true,
  ...
  int minSizeSq = params.minSize * params.minSize,
      minSize4 = 4 * params.minSize;
  contours = visitedAwareTracingContour(thresImage, minSize4,
                                        params.maxTimesRevisited);

  // decide where to store the candidates. If candidatesOut is not null, store
  // there, otherwise use a local variable
  std::vector<FiducialMarker> candidateslocal;
  if (candidatesOut != nullptr) {
    candidatesOut->clear();
  } else {
  ...
  for (size_t i = 0; i < contours.size(); i++) {
    ...
    cv::approxPolyDP(contours[i], approxCurve,
                     double(contours[i].size()) * 0.03, true);
    ...
    for (int j = 0; j < 4; j++)
      marker.corners.emplace_back(
          cv::Point2f(float(approxCurve[j].x), float(approxCurve[j].y)));
  ...
  for (size_t di = 0; di < dictionaries.size(); di++) {
    ...
    cv::Mat bits(dictInstance.markerSize + 2 * params.markerBorderBits,
                 dictInstance.markerSize + 2 * params.markerBorderBits,
                 CV_8UC1),
        bitadaptive(dictInstance.markerSize + 2 * params.markerBorderBits,
                    dictInstance.markerSize + 2 * params.markerBorderBits,
                    CV_8UC1);
    ...
    for (auto it = candidatesOut->begin(); it != candidatesOut->end();) {
      ...
      ////// extract the code. Obtain the intensities of the bits using
      ///homography
      for (int i = 0;
           i < int(params.maxAttemptsPerCandidate) && marker.id == -1; i++) {
        // if not first attempt, we may wanna produce small random alteration of
        // the corners
        auto marker2 = marker;
        if (i != 0)
          for (int c = 0; c < 4; c++) {
            marker2.corners[c].x += rand.gaussian(0.75);
            marker2.corners[c].y += rand.gaussian(0.75);
          } // if not first, alter corner location
        ...
        if (i ==
            2) { // if not working the first time, try this time adaptive
                 // threshold into the bits to improve robustness to lighting
          thres255Adaptive(bits, bitadaptive);
          bitadaptive.copyTo(bits);
        } else {
          cv::threshold(bits, bits, 0, 255, cv::THRESH_OTSU);
        }
        // now, analyze the inner code to see it if is a marker. If so, rotate
        // to have the points properly sorted
        int nrotations = 0;
        if (getMarkerId(bits, marker.id, nrotations, params, dictInstance) == 0)
          continue;
        std::rotate(marker.corners.begin(),
                    marker.corners.begin() + 4 - nrotations,
                    marker.corners.end());
      }
      if (marker.id != -1) {
    ...
    /// REMOVAL OF INNER DUPLICATED DETECTIONS OF THE SAME MARKER(INNER AND
    /// OUTER BORDER)
    std::sort(currDirMarkerDetected.begin(), currDirMarkerDetected.end(),
              [](const FiducialMarker &a, const FiducialMarker &b) {
                return a.id < b.id;
              });
    {
      ...
      for (int i = 0; i < int(currDirMarkerDetected.size()) - 1; i++) {
        for (int j = i + 1;
             j < int(currDirMarkerDetected.size()) && !toRemove[i]; j++) {
          if (currDirMarkerDetected[i].id == currDirMarkerDetected[j].id) {
            auto res = isInto(currDirMarkerDetected[i].corners,
                              currDirMarkerDetected[j].corners);
            if (res == 1)
              toRemove[i] = true;
            else if (res == 2)
              toRemove[j] = true;
      ...
      for (size_t i = 0; i < currDirMarkerDetected.size(); i++)
        if (!toRemove[i])
          DetectedMarkers.push_back(currDirMarkerDetected[i]);
    }
  }
  ...
  if (DetectedMarkers.size() > 0) {
    ...
    for (size_t i = 0; i < DetectedMarkers.size(); i++)
      for (int c = 0; c < 4; c++)
        DetectedMarkers[i].corners[c] = Corners[i * 4 + c];
  }
 ...
 *                           1 if the candidate is a black candidate (default
 * candidate) 2 if the candidate is a white candidate
 */
int MarkerDetector::getMarkerId(cv::Mat candidateBits, int &idx,
                                int &nrotations,
                                const DetectionParameters &params,
                                Dictionary &dictionary) {
  uint8_t typ = 1;
  ...
  int maximumErrorsInBorder =
      int(dictionary.markerSize * dictionary.markerSize *
  ...
  cv::Mat onlyBits =
      candidateBits
          .rowRange(params.markerBorderBits,
                    candidateBits.rows - params.markerBorderBits)
          .colRange(params.markerBorderBits,
                    candidateBits.cols - params.markerBorderBits);
  onlyBits /= 255;
  // try to indentify the marker
  if (!dictionary.identify(onlyBits, idx, nrotations,
                           params.errorCorrectionRate))
    return 0;
  return typ;
}
/**
 * @brief Return number of erroneous bits in border, i.e. number of white bits
 * in border.
 */
int MarkerDetector::getBorderErrors(const cv::Mat &bits, int markerSize,
                                    int borderSize) {
...
float MarkerDetector::getSubpixelValue(const cv::Mat &im_grey,
                                       const cv::Point2f &p) {
  ...
  // We check ix+1 and iy+1 because the interpolation looks at the next pixel
  // over.
  ...
  const float bot =
      p10 + dx * (p11 - p10); // Interpolate Bottom Row Horizontallys
...
FiducialMarker MarkerDetector::sort(const FiducialMarker &marker) {
  ...
  if (o < 0.0)
    std::swap(res_marker.corners[1], res_marker.corners[3]);
  return res_marker;
}

/**
 * @brief Traces the contours of a binary image using our visited aware Tracing
 * algorithm.
 *
...
MarkerDetector::visitedAwareTracingContour(cv::Mat &padded_io, size_t minSize,
                                           float maxRevisited) {
  if (padded_io.empty() || padded_io.type() != CV_8UC1)
    return {};
  ...
  // 2. Precompute Neighbor Offsets based on image stride This removes the need
  // for Point arithmetic in the loop
  const int offsets[16] = {
      -1, -step - 1, -step, -step + 1, 1, step + 1, step, step - 1,
      -1, -step - 1, -step, -step + 1, 1, step + 1, step, step - 1,
  };
  // Use static tables to avoid initialization overhead on every call //
  // 8-connectivity offsets relative to center (0,0) // Order: W, NW, N, NE, E,
  // SE, S, SW
      ...
      if (row_ptr[c] == FOREGROUND) { // --- 4. Tracing Loop  if is foreground
        ...
        do {
          ...
          *curr_ptr = VISITED;                 // Mark as visited
          // showImage(padded);
          //  Search for next foreground pixel. We search 8 neighbors starting
          //  from search_idx
          for (int i = 0; i < 8; ++i) {
            int idx = search_idx + i; // index into offsets (0..15)
            uchar *neighbor =
                curr_ptr + offsets[idx]; // Fast pointer arithmetic
            if (*neighbor != BACKGROUND) {
              // Found next boundary pixel
              curr_ptr = neighbor;
              int dir =
                  (idx &
                   7); // Update Integer Coordinates using the small static
                       // tables(Use modulo 8 to get the distinct direction 0-7)
...
void MarkerDetector::thres255Adaptive(cv::Mat &in, cv::Mat &out, int off,
                                      int thres) {
...
std::vector<FiducialMarker> detect(DictionaryType dictionary, cv::Mat &src_gray,
                                   cv::Mat &thresImage, int erosionIt) {

  cv::Mat kernel = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(3, 3));
  cv::erode(thresImage, thresImage, kernel, {-1, -1}, erosionIt);
  DetectionParameters params;
  return MarkerDetector::detect(src_gray, {dictionary}, params, nullptr,
                                thresImage);
}
...
int cornerMaxDistance(const FiducialMarker &m1, const FiducialMarker &m2) {
  int md = 0;
  for (int i = 0; i < 4; i++) {
    int d = int((m1.corners[i].x - m2.corners[i].x) *
                    (m1.corners[i].x - m2.corners[i].x) +
                (m1.corners[i].y - m2.corners[i].y) *
                    (m1.corners[i].y - m2.corners[i].y));
    if (d > md)
      md = d;
  }
...
std::vector<FiducialMarker> detectBWMarkers(cv::Mat &src_gray,
  ...
  // BLACK MARKERS
  cv::boxFilter(src_gray, thresImage, src_gray.type(), cv::Size(25, 25),
                cv::Point(-1, -1), true,
                cv::BORDER_REPLICATE | cv::BORDER_ISOLATED);
  thresImage = thresImage - src_gray;
  cv::threshold(thresImage, thresImage, 3, 255, cv::THRESH_BINARY);
  // determine how many erosion iterations we will do, depending on the size of
  // the image
  ...
  cv::parallel_for_(range, [&](const cv::Range &r) {
    for (int i = r.start; i < r.end; i++) {
      ...
      markers_blackv[i] =
          detect(dictionary, src_gray, thres, i); // black markers
      // because we have shrink the borders for black markers, we will expand
      // the corners a bit from the center
  ...
  if (markers_blackv.size() > 1) {
    ...
    /// REMOVAL OF INNER DUPLICATED DETECTIONS OF THE SAME MARKER(INNER AND
    /// OUTER BORDER)
    std::sort(markers_black.begin(), markers_black.end(),
              [](const FiducialMarker &a, const FiducialMarker &b) {
                return a.id < b.id;
              });
    ...
    for (unsigned int i = 0; i < markers_black.size(); i++)
      if (!toRemove[i])
        unduplicated.push_back(markers_black[i]);
    markers_black = unduplicated;
  }

  //    markers_black=detect(board.dictionary,src_gray,thresImage,erodeIterations);//black
  //    markers

  /// WHITE MARKERS
  cv::boxFilter(src_gray, thresImage, src_gray.type(), cv::Size(15, 15),
                cv::Point(-1, -1), true,
                cv::BORDER_REPLICATE | cv::BORDER_ISOLATED);
  thresImage = thresImage - src_gray;
  cv::threshold(thresImage, thresImage, 3, 255, cv::THRESH_BINARY);

  // draw a line between opposite corners to break the continous contour of the
  // white markers, which will help to detect them.
  ...
  markers_white =
      detect(dictionary, src_gray_inv, thresImage, 1); // white markers

  // Combine results from both
  std::vector<FiducialMarker> allMarkers;
  allMarkers.reserve(markers_black.size() + markers_white.size());
  allMarkers.insert(allMarkers.end(), markers_black.begin(),
                    markers_black.end());
  allMarkers.insert(allMarkers.end(), markers_white.begin(),
                    markers_white.end());
...
std::shared_ptr<cv::flann::Index>
buildFlannIndex(const std::vector<FiducialMarker> &markers) {
...
// returns a set of markers that are connected, i.e. that have at least one
// corner closer than a threshold.
std::vector<std::vector<FiducialMarker>>
connectedMarkerComponents(const std::vector<FiducialMarker> &markers) {
  // create a vector with all corners
  // now, create a flann index with these corners
  ...
  for (size_t m = 0; m < markers.size(); m++) {
    for (int i = 0; i < 4; i++) {
      ...
      cv::Mat query = (cv::Mat_<float>(1, 2) << markers[m][i].x,
                       markers[m][i].y); // Single 2D query point
      int n = flannIndex->radiusSearch(
          query, indices, dists, threshold * threshold, 4 * markers.size());
      for (int x = 0; x < n; x++) {
        int idx = indices[x];
...
// given a marker id and one of its corners, return the global corner id of that
// corner, which is a unique id for that corner in the whole board,

int getGlobalCornerID(int marker_id, int corner_id, const cv::Size &bSize,
...
// opposite of getGlobalCornerID, given a global corner id, return the marker
// ids and corner ids of that corner

std::vector<std::pair<int, int>>
getMarkerCornersFromGlobalCornerID(int gid, cv::Size bSize,
                                   const std::vector<int> &ids) {
  auto getId = [&](int row, int col) -> int {
    if (row < 0 || row >= bSize.height || col < 0 || col >= bSize.width)
      return -1;
  ...
  // Board.getId() returns -1 for out-of-range (row,col), so it doubles as a
  // bounds check.
  int id;
...
std::vector<FiducialMarker>
detectFiducialMarkers(InputArray image,
                      const std::vector<DictionaryType> &dictionaries,
                      const DetectionParameters &detectorParams) {
  CV_Assert(!image.empty());
  if (image.isUMat() && cv::ocl::useOpenCL()) {
    return MarkerDetector::detect(image.getUMat(), dictionaries,
                                  detectorParams);
  }
...
std::vector<FiducialMarker>
detectFiducialMarkers(InputArray image, DictionaryType dictionary,
                      const DetectionParameters &detectorParams) {
  return detectFiducialMarkers(image, std::vector<DictionaryType>{dictionary},
                               detectorParams);
}

void getFiducialMarkerImage(OutputArray _img, DictionaryType dictionary, int id,
                            int bitSize, bool externalBorder) {
  ...
  cv::resize(markerWithBorders, _img, cv::Size(sidePixels, sidePixels), 0, 0,
             cv::INTER_NEAREST);
}

void drawFiducialMarkers(InputOutputArray _image,
                         const std::vector<FiducialMarker> &markers,
                         Scalar borderColor) {
  cv::Mat image = _image.getMat();
  Scalar cornerColor(255 - borderColor[0], borderColor[1], borderColor[2]);
  Scalar textColor(255 - borderColor[0], 255 - borderColor[1],
                   255 - borderColor[2]);
  ...
  for (const auto &marker : markers) {
    ...
    // draw 4 sides
    for (int j = 0; j < 4; j++)
      cv::line(image, cv::Point(marker.corners[j]),
               cv::Point(marker.corners[(j + 1) % 4]), borderColor, 1);
    ...
    for (const auto &c : marker.corners)
      center += c;
    center *= 0.25f;
...
void drawAxis(InputOutputArray image, InputArray cameraMatrix,
              InputArray distCoeffs, InputArray rvec, InputArray tvec,
              float length) {
  std::vector<cv::Point3f> axisPoints = {{0.f, 0.f, 0.f},
                                         {length, 0.f, 0.f},
                                         {0.f, length, 0.f},
                                         {0.f, 0.f, length}};
...
void getSolvePnpPoints(const FiducialMarker &marker, OutputArray objPoints,
...
bool detectGridBoard(InputArray image, cv::Size gridSize,
                     DictionaryType dictionary, CV_OUT GridBoard &board_,
                     InputArray _ids) {
  ...
  // if ids is empty, we will consider all the ids in the dictionary up to
  ...
  std::vector<std::vector<FiducialMarker>> connected_markers =
      connectedMarkerComponents(allMarkers);

  // lets detect possible inconsistencies, i.e., markers in the wrong order,
  // possibly belonging to another board configuration
  ...
  for (auto &comp : connected_markers) {
    ...
    // for each corner, of each marker we will analyze its nearst neighbor. If
    // is really connected, we will check if the connection is consistent
    for (const auto &marker : comp) {
      for (int c = 0; c < 4; c++) {
        std::vector<int> indices;
        std::vector<float> dists;
        cv::Mat query = (cv::Mat_<float>(1, 2) << marker[c].x,
                         marker[c].y); // Single 2D query point
        int nn = findex->radiusSearch(query, indices, dists,
                                      threshold * threshold, marker.size());
        nn = std::min(nn, int(indices.size())); // flann bug in <4.13
        for (int ix = 0; ix < nn; ix++) {
          int idx = indices[ix];
          if (comp[idx / 4].id == marker.id)
            continue; // same marker
          // check if the connection is consistent, i.e., if the global corner
          // ids are the same
          int gid1 = getGlobalCornerID(marker.id, c, gridSize, ids);
          int gid2 =
              getGlobalCornerID(comp[idx / 4].id, idx % 4, gridSize, ids);
          if (gid1 != gid2) {
            CV_LOG_WARNING(nullptr,
                           "Marker "
                               << marker.id << " corner " << c
                               << " connected to marker " << comp[idx / 4].id
                               << " corner " << (idx % 4)
                               << " but global corner ids differ: " << gid1
                               << " vs " << gid2);
            is_consistent = false;
          }
        }
  ...
  if (consistent_connected_markers.empty())
    return false;
  ...
  if (connected_markers_filtered.empty())
    return false;
  allMarkers = connected_markers_filtered;

  // now, lets focus on the global corners. Unify the corners for subpixel
  // analysis first, obtain the average position for each global corner id found
  ...
  // compute the window size for the subpixel refinement based on the size of
  // the markers. Bigger makers, bigger search zone
  double avrgLen = 0;
  for (const auto &m : allMarkers) {
    for (int i = 0; i < 4; i++) {
  ...
  // refine the corners
  cv::cornerSubPix(
      src_gray, Corners, cv::Size(halfwsize, halfwsize), cv::Size(-1, -1),
      cv::TermCriteria(cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 12,
                       0.005));
  ...
  for (auto it_gc = global_corners.begin(); it_gc != global_corners.end();
       ++it_gc) {
    int gid = it_gc->first;
    cv::Point2f corner_c = it_gc->second.first;
    // find in how many markers it is involved
    auto marker_corners =
        getMarkerCornersFromGlobalCornerID(gid, gridSize, ids);
    for (auto it_mc = marker_corners.begin(); it_mc != marker_corners.end();
         ++it_mc) {
      int markerid = it_mc->first;
      int cornerid = it_mc->second;
      // see if the makrer id is detected
      auto markerid_copy = markerid; // capturing markerid only in C++20, lets
                                     // use a copy for prev compilers
      auto it = std::find_if(allMarkers.begin(), allMarkers.end(),
                             [markerid_copy](const FiducialMarker &m) {
                               return m.id == markerid_copy;
                             });
...
void getGridBoardImage(OutputArray img, Size bSize, DictionaryType dictionary,
  ...
  if (nmarkers > dictInstance.bytesList.rows)
    CV_Error(
        cv::Error::StsBadArg,
        "Number of markers exceeds the number of markers in the dictionary");
  ...
  cv::Size imgSize(markerSizePix * bSize.width + 2 * border,
                   markerSizePix * bSize.height + 2 * border);
  ...
  for (int y = 0; y < bSize.height; y++) {
    ...
    for (int x = 0; x < bSize.width; x++, markerIdx++) {
      cv::Mat markerImg;
      dictInstance.generateImageMarker(ids[markerIdx], markerSizePix,
                                       markerImg);
      ...
      markerImg.copyTo(
          outImage(cv::Rect(posX, posY, markerSizePix, markerSizePix)));
  ...
  for (int x = 1; x < bSize.width; x += 2)
    cv::rectangle(
        outImage,
        cv::Rect(border + x * markerSizePix, 0, markerSizePix, border),
        cv::Scalar::all(0), cv::FILLED);
  for (int x = bSize.height % 2; x < bSize.width; x += 2)
    cv::rectangle(outImage,
                  cv::Rect(border + x * markerSizePix,
                           border + markerSizePix * bSize.height, markerSizePix,
                           border),
                  cv::Scalar::all(0), cv::FILLED);
  for (int y = 1; y < bSize.height; y += 2)
    cv::rectangle(
        outImage,
        cv::Rect(0, border + y * markerSizePix, border, markerSizePix),
        cv::Scalar::all(0), cv::FILLED);
  for (int y = bSize.width % 2; y < bSize.height; y += 2)
    cv::rectangle(outImage,
                  cv::Rect(border + markerSizePix * bSize.width,
                           border + y * markerSizePix, border, markerSizePix),
                  cv::Scalar::all(0), cv::FILLED);

  cv::rectangle(outImage, cv::Rect(0, 0, border, border), cv::Scalar::all(0),
                cv::FILLED);
  cv::rectangle(outImage, cv::Rect(outImage.cols - border, 0, border, border),
                cv::Scalar::all(0), cv::FILLED);
  cv::rectangle(outImage, cv::Rect(0, outImage.rows - border, border, border),
                cv::Scalar::all(0), cv::FILLED);
  cv::rectangle(
      outImage,
      cv::Rect(outImage.cols - border, outImage.rows - border, border, border),
      cv::Scalar::all(0), cv::FILLED);
  // copy result to img
  outImage.copyTo(img);
}
void drawGridBoard(InputOutputArray image, const GridBoard &board, Scalar color,
                   bool drawMarkerIds) {
  for (size_t i = 0; i < board.detectedBoardCorners.size(); i++) {
    ...
    putText(image, s.str(),
            board.detectedBoardCorners[i].second - cv::Point2f(15, 10),
  ...
  if (drawMarkerIds) {
    for (auto m : board.markers) {
      ...
      std::stringstream s;
      s << m.id;
      putText(image, s.str(), center, FONT_HERSHEY_SIMPLEX, 0.5,
              cv::Scalar(255, 255, 255) - color, 2);
    }
  }
}

void getSolvePnpPoints(const GridBoard &board, OutputArray objPoints,
                       OutputArray imgPoints, float markerSize) {
...
void getDiamondImage(OutputArray img, const DictionaryType &dictionary,
                     const cv::Vec4i &ids, int bitSize) {
  getGridBoardImage(img, cv::Size(2, 2), dictionary, bitSize, ids);
}

std::vector<Diamond> detectDiamonds(InputArray image,
                                    DictionaryType dictionary) {
  CV_Assert(image.channels() == 1 || image.channels() == 3);
  cv::Mat src_gray;
  // obtain the gray image
  ...
  for (const auto &comp : connected_markers) {
    ...
    std::pair<int, int>
        centralCorner; // this is the central poisition, that let us know the
                       // order of the markers in the diamond
    for (size_t m = 0; m < comp.size(); m++) {
      ...
      for (int c = 0; c < 4; c++) {
        ...
        //  std::cout<<"nn="<<nn<<" for marker "<<marker.id<<" corner
        //  "<<c<<std::endl;
    ...
    // std::cout<<"Component with markers
    // "<<comp[0].id<<","<<comp[1].id<<","<<comp[2].id<<","<<comp[3].id<<" has
    // "<<totalSharedCorners<<" shared corners"<<std::endl; std::cout<<"Central
    // corner is marker "<<centralCorner.first<<" corner
    // "<<centralCorner.second<<" with "<<cornerWithMostSharedCorners<<" shared
    // corners"<<std::endl;
    if (totalSharedCorners != 20)
      continue;

    // find the diamon id eximiing the poition of the central corner, which is
    // the one with more shared corners.
    cv::Vec4i diamondIds;
    ...
    for (size_t m = 0; m < comp.size(); m++) {
      for (int c = 0; c < 4; c++) {
        auto gid =
            getGlobalCornerID(comp[m].id, c, cv::Size(2, 2),
                              std::vector<int>{diamondIds[0], diamondIds[1],
                                               diamondIds[2], diamondIds[3]});
        board_corners[gid] = comp[m][c];
      }
    }

    // lets apply a corner refinement to the corners. first, estimate average
    // lenght
    ...
    // now, subpix
    cv::cornerSubPix(
        src_gray, board_corners, cv::Size(halfwsize, halfwsize),
        cv::Size(-1, -1),
        cv::TermCriteria(cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 12,
                         0.005));
...
void drawDiamonds(InputOutputArray image, const std::vector<Diamond> &diamonds,
  ...
  for (const auto &d : diamonds) {
    ...
    for (size_t i = 0; i < d.corners.size(); i++) {
      // draw a rect
      cv::rectangle(image,
                    cv::Rect(d.corners[i] - cv::Point2f(3, 3), cv::Size(6, 6)),
                    color, -1);
      center += d.corners[i];
    }
    ...
    std::stringstream s;
    s << "id=" << d.id;
    putText(image, s.str(), center - cv::Point2f(50, 10), FONT_HERSHEY_SIMPLEX,
            0.5, color, 2);
    ...
    if (drawMarkerIds) {
      for (auto m : d.markers) {
        ...
        for (const auto &c : m.corners)
          m_center += c;
        m_center *= 0.25f;
        putText(image, ss.str(), m_center, FONT_HERSHEY_SIMPLEX, 0.5,
                cv::Scalar(255, 255, 255) - color, 2);
      }
    }
  }
}
void getSolvePnpPoints(const Diamond &diamond, OutputArray objPoints,
                       OutputArray imgPoints, float markerSize) {
